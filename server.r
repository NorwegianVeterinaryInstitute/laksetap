server <- function(input, output) {
  ...
  output$top_bar <- renderUI({
    if (
      input$navbar == "monthly_mortality" |
  ...
  output$calc_banner <- renderUI({
    if (input$calc_nav == "calc") {
      tagList(
        shiny::div(
          style = "padding-left: 1rem; padding-top: 3rem;",
          ...
          bslib::layout_column_wrap(
            ...
            numericInput(
              "end_count",
              "Antall fisk ved periodens slutt",
              value = 90,
              min = 0,
              max = 200000
            ),
            numericInput(
              "dead_count",
              "Antall døde fisk i løpet av perioden",
              value = 5,
              min = 0,
              max = 200000
            ),
  ...
  output$table_cohort <- DT::renderDT({
    if (input$geo_group == "all") {
      df_cohorts() %>%
        dplyr::filter(
          year %in% input$select_years_table5 & area == "Norge"
        ) %>%
      ...
      df_cohorts() %>%
        dplyr::filter(
          year %in% input$select_years_table5 & area %in% input$select_locs
        ) %>%
        dplyr::select(year, area, q1, median, q3) %>%
        datatable(
          # filter = "top",
  ...
  observeEvent(input$calculate_button_cum, {
    nums <- suppressWarnings(as.numeric(unlist(strsplit(
      input$mortality_input_cum,
      ","
    ))))
    ...
    if (any(is.na(nums))) {
      showNotification(
        "Én eller flere inndata er ikke numeriske.",
        type = "error"
      )
      return()
    }
    ...
    if (!all(between_result)) {
      showNotification(
        "Dødsrate kan ikke være lavere enn 0, eller høyere enn 2.",
        type = "error"
      )
      return()
    }
    ...
    period_label <- switch(input$period_type, "week" = "Uke", "month" = "Måned")
    ...
    output$cumulative_risk_plot <- renderPlot({
      ...
      barplot(
        mort_rates_cum * 100,
        names.arg = seq_along(mort_rates_cum),
        col = "#1C4FB9",
        main = "",
        xlab = period_label,
        ylab = "Dødsrate (%)"
    ...
    output$result_text_cum <- renderText({
      paste(
        "Dødelighet for den siste registrerte perioden",
        sprintf("%.2f%%", tail(cum_risks * 100, 1))
      )
    })
  })
}
