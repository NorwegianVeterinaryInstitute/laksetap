...
server <- function(input, output, session) {
  ...
  observeEvent(input$add_entry_button, {
    req(
      !is.na(input$year),
      !is.na(input$period_value),
      !is.na(input$beginning_count),
      !is.na(input$end_count),
      !is.na(input$dead_count)
    )
    ...
    if (ar_count == 0) {
      showNotification(
        "Beginning and end counts cannot both be zero.",
        type = "error"
      )
      return()
    }
    ...
    period_str <- paste0(
      input$year,
      "-",
      sprintf("%02d", as.integer(input$period_value))
    )

    existing <- mortality_data()
    if (period_str %in% existing$Period) {
      showNotification(
        "This period already has data. Please choose another.",
        type = "error"
      )
      return()
    }
  ...
  output$mortality_plots <- renderPlot({
    ...
    text(
      x = length(cum_risks),
      y = latest_cum * 100,
      labels = sprintf("Latest: %.2f%%", latest_cum * 100),
      pos = 3,
      col = "blue"
    )
  })
